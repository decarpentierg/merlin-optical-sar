    ...
    def __getitem__(self, index):
        ...
        if not isinstance(self.files, list):
            ...
            for file in self.files:
                im = np.load(file)
                eval_data.append(np.array(im).reshape(1, np.size(im, 0), np.size(im, 1), np.size(im, 2)))
        ...
        image_imag_part = (current_test[:, :, :, 1]).reshape(current_test.shape[0], current_test.shape[1],
                                                              current_test.shape[2], 1)
        if self.method != 'SAR':
            image_sup = (current_test[:, :, :, 2:]).reshape(current_test.shape[0], current_test.shape[1],
                                                                    current_test.shape[2], current_test.shape[3]-2)
            # normalize image_sup on each channel
            for i in range(image_sup.shape[-1]):
                image_sup[:, :, :, i] = (image_sup[:, :, :, i] - np.min(image_sup[:, :, :, i])) / (np.max(image_sup[:, :, :, i]) - np.min(image_sup[:, :, :, i]))
        else:
            image_sup = 0

        return torch.tensor(image_real_part).type(torch.float) , torch.tensor(image_imag_part).type(torch.float), torch.tensor(image_sup).type(torch.float)
