...
class AE(torch.nn.Module):
    ...
    def __init__(self, batch_size, eval_batch_size, device, method):
        ...
        if method != "SAR":
            self.enc0_opt = torch.nn.Conv2d(
                in_channels=self.n_channels_sup, out_channels=48, **conv_kwargs
            )
    ...
    def forward(self, x, batch_size):
        ...
        if self.method != "SAR":
            # List for skip
            x_sup = torch.reshape(
                x[:, :, :, :, 1:], [batch_size, self.n_channels_sup, 256, 256]
            )
            skips_sup = [x_sup]
    ...
    def loss_function(self, output, target, batch_size):
        ...
        loss: float
        """
        log_hat_R = 2 * (output * (MAX - MIN) + MIN)
    ...
    def training_step(self, batch, batch_number):
        ...
        input = (torch.log(torch.square(known) + 1e-3) - 2 * MIN) / (2 * (MAX - MIN))
    ...
    def validation_step(self, batch, image_num, epoch_num, eval_files, eval_set, sample_dir):
        ...
        image_real_part_normalized = (
            torch.log(torch.square(image_real_part) + 1e-3) - 2 * MIN
        ) / (2 * (MAX - MIN))
        image_imaginary_part_normalized = (
            torch.log(torch.square(image_imaginary_part) + 1e-3) - 2 * MIN
        ) / (2 * (MAX - MIN))

        if self.method == "SAR":
            out_real = self.forward(
                torch.unsqueeze(image_real_part_normalized, -1), self.eval_batch_size
            )
            out_imaginary = self.forward(
                torch.unsqueeze(image_imaginary_part_normalized, -1),
                self.eval_batch_size,
            )
        else:
            out_real = self.forward(
                torch.cat((image_real_part_normalized, im), -1), self.eval_batch_size
            )
            out_imaginary = self.forward(
                torch.cat((image_imaginary_part_normalized, im), -1),
                self.eval_batch_size,
            )
        ...
        noisyimage = np.squeeze(
            np.sqrt(
                np.square(image_real_part.cpu().numpy())
                + np.square(image_imaginary_part.cpu().numpy())
            )
        )
    ...
    def test_step(self, im, image_num, test_files, test_set, test_dir):
        ...
        for x in x_range:
            for y in y_range:
                patch_test_real = image_real_part_normalized[
                    :, x : x + pat_size, y : y + pat_size, :
                ]
                patch_test_imag = image_imaginary_part_normalized[
                    :, x : x + pat_size, y : y + pat_size, :
                ]
                patch_test_opt = opt[:, x : x + pat_size, y : y + pat_size, :]

                tmp_real = self.forward(
                    torch.cat((patch_test_real, patch_test_opt), dim=2),
                    self.eval_batch_size,
                )
                tmp_imag = self.forward(
                    torch.cat((patch_test_real, patch_test_opt), dim=2),
                    self.eval_batch_size,
                )

                tmp = 0.5 * (
                    np.square(denormalize_sar(tmp_real.cpu().numpy()))
                    + np.square(denormalize_sar(tmp_imag.cpu().numpy()))
                )

                out[x : x + pat_size, y : y + pat_size] = (
                    out[x : x + pat_size, y : y + pat_size] + tmp
                )

                count_image[x : x + pat_size, y : y + pat_size] = count_image[
                    x : x + pat_size, y : y + pat_size
                ] + np.ones((pat_size, pat_size))
