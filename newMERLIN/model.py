...
class AE(torch.nn.Module):
    ...
    def __init__(self,batch_size,eval_batch_size,device):
        ...
        # add layers to process the optic image
        ############################################################################################################

        self.enc0_opt = torch.nn.Conv2d(in_channels=3, out_channels=48, kernel_size=(3, 3), stride=(1, 1),
                                    padding='same')
        ...
        self.dec2b = torch.nn.Conv2d(in_channels=96, out_channels=96, kernel_size=(3, 3), stride=(1, 1),
                                     padding='same')
        self.dec1a = torch.nn.Conv2d(in_channels=99, out_channels=64, kernel_size=(3, 3), stride=(1, 1),
                                     padding='same')
    ...
    def forward(self,x ,batch_size):
        ...
        ############################################################################################################
        # ENCODER OPTIC 
        x_OPT = torch.reshape(x[:, :, :,:, 1:], [batch_size, 3, 256, 256])
        skips_OPT = [x_OPT]

        n_OPT = x_OPT

        n_OPT = self.leaky(self.enc0_opt(n_OPT))
        n_OPT = self.leaky(self.enc1_opt(n_OPT))
        n_OPT = self.pool(n_OPT)
        skips_OPT.append(n_OPT)

        n_OPT = self.leaky(self.enc2_opt(n_OPT))
        n_OPT = self.pool(n_OPT)
        skips_OPT.append(n_OPT)

        n_OPT = self.leaky(self.enc3_opt(n_OPT))
        n_OPT = self.pool(n_OPT)
        skips_OPT.append(n_OPT)

        n_OPT = self.leaky(self.enc4_opt(n_OPT))
        n_OPT = self.pool(n_OPT)
        skips_OPT.append(n_OPT)

        n_OPT = self.leaky(self.enc5_opt(n_OPT))
        n_OPT = self.pool(n_OPT)
        n_OPT = self.leaky(self.enc6_opt(n_OPT))

        # COMBINE
        #n = torch.cat((torch.unsqueeze(torch.mean(n_SAR, 2), dim=2), torch.unsqueeze(torch.mean(n_OPT, 2), dim=2)), dim=2)
        n = n_SAR + n_OPT
        ...
        # DECODER
        n = self.upscale2d(n)
        skips = skips_SAR.pop()+ skips_OPT.pop()
