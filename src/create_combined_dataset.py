...
if args.dataset == "Saclay":
    ...
    opt_no_register = mvalab.imz2mat("data/Saclay/TelecomParisOPT.IMA")[0][
        :, :, 0
    ]  # only the first image is relevant
    di, dj = -19, 147  # define registering translation
else:
    opt_no_register = mvalab.imz2mat("data/Sendai/PileTSX_AVANT_surTSX_1024x1024RECALOPT.IMA")[
        0
    ][:988, 33:]
    sar_no_register = mvalab.imz2mat("data/Sendai/PileTSX_AVANT_surTSX_1024x1024RECALZ4.IMA")[
        0
    ][:988, 33:, :]
...
for i in range(sar.shape[-1]):
    for j in range(l // 256):
        for k in range(L // 256):
            ...
            np.save(
                folder_name
                + "/validation/spotlight/val_data_"
                + str(i)
                + "_"
                + str(j)
                + "_"
